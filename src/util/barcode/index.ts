...
function calculateUpcCheckDigit(bc: string) {
    ...
    const sum = digits.map((item, index) => parseInt(item, 10) * UPC_MULTIPLIERS[index]).reduce((pv, cv) => pv + cv, 0);
    let cd = 10 - (sum % 10);
...
function calculateIsbnCheckDigit(bc: string) {
    ...
    const sum = digits.map((item, index) => parseInt(item, 10) * ISBN_MULTIPLIERS[index]).reduce((pv, cv) => pv + cv, 0);
    let cd = 11 - (sum % 11);
...
export function checkCheckDigit(bc: string) {
    const checkdigit = bc[bc.length - 1];
    const upcCD = calculateUpcCheckDigit(bc);
    if (upcCD === checkdigit) {
        const type = unpadZero(bc).length === 13 ? ['978', '979'].some(x => bc.startsWith(x)) ? 'isbn-13' : 'ean' : unpadZero(bc).length === 12 ? unpadZero(bc).startsWith('40') ? 'sku' : unpadZero(bc).startsWith('49') ? 'locator' : 'upc' : 'upc';
        return [true, type];
    }
...
function convertIsbn10To13(bc: string) {
    ...
    const cd = calculateUpcCheckDigit('978'.concat(sansCD.padStart(9, '0')).concat('0'));
